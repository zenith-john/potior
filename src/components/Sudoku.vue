...
export default {
  data () {
    return {
      puzzle: Array.from({ length: 9 }, () => Array(9).fill({ value: 0, original: false })) as { value: number; original: boolean; }[][],
      activeRow: -1,
      activeCol: -1,
      seconds: 0,
      timer: 0
  ...
  methods: {
    generatePuzzle () {
      ...
      this.puzzle = board_string_to_grid(boardString)
        .map(row => {
          return row.map(cell => {
            return {
  ...
  <div class="sudoku">
    ...
    <div class="grid">
      ...
      >
        ...
        >
          ...
          <template v-else>
            <div class="srow" v-for="srow in [0,1,2]">
              <div class="scell" v-for="scol in [0,1,2]">
    ...
    <div class="row">
      <button
        type="button"
        class="btn"
        v-for="value in Array(10).keys()" :key="value"
