...
export default {
  ...
  methods: {
    ...
    binary(code: string) {
      const gap = 5
      const trim_code = code.replace(/\s/g, '')
      if (trim_code.length % gap != 0) {
        return 'Invalid as binary code'
      } else {
        let ret = ''
        const shift = 'a'.charCodeAt(0) - 1
        for (let i = 0; i < trim_code.length; i += gap) {
    ...
    morse(code: string) {
      const morsify = code.replace(/1/g, '-').replace(/0/g, '.')
      const codes = morsify.split(' ')
      let ret = ''
...
<template>
  <label for="code">
    ...
    <template v-if="!isValid"> Invalid input. </template>
  </label>
  <div>
